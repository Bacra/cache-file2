...
function addCache(file, lockfile, lockOpts, content, callback) {
	...
	lockFile.lock(lockfile, lockOpts, function(err) {
		...
		if (content === null) {
			...
			cacheContent(typeof content == 'string' ? new Buffer(content) : content, callback);
		}
	});	
}
...
function addCacheSync(file, lockfile, lockOpts, content) {
	...
	return content === null ? cacheContent : cacheContent(typeof content == 'string' ? new Buffer(content) : content);
}
